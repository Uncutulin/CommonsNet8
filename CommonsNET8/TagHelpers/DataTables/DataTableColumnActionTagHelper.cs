...
using CommonsNET8.TagHelpers.DataTables;
...
namespace CommonsNET8.TagHelpers.DataTables
{
    ...
    public class DataTableColumnActionTagHelper : TagHelper
    {
        ...
        private const string ActionTitleName = "title";
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            if (Visible)
            {
                ...
                dataTableContext.ActionDataSet.Add(new ActionData
                {
                    ActionTitle = ActionTitle, ActionUrl = ActionUrl, CanExecuteProperty = CanExecuteProperty?.Name?.ToLower(), Classes = Classes, Content = output.GetChildContentAsync().Result.GetContent() 
                });
